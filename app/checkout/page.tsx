...
    // Load profile data (asumo que 'profiles' tiene full_name)
...
    try {
      // Preparamos los ítems (incluyendo 'name' para Mercado Pago)
      const itemsParaAPI = items.map((item) => ({
        product_id: item.product.id,
        quantity: item.quantity,
        price: item.product.price,
        name: item.product.name, // ⬅️ Nombre para MP
      }))
...
        },
        userEmail: user?.email, // ⬅️ Email del pagador para MP
      }

      // 1. Llamada al servidor para crear la orden en Supabase y la Preferencia MP
...
      // 2. Extraer el link de pago
      const { init_point } = await response.json()

      // 3. Redirección final a Mercado Pago
      if (init_point) {
        window.location.href = init_point // ⬅️ Redirección directa a la pasarela MP
      } else {
